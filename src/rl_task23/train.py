...
from pipe import EBDataPipe, Task3EBDataPipe
from model import EBRLModel
from utils import EBRLMetric, ScheduledSamplingCallback, EBPredMetric, EBPredMetricWithPara, Task3EBPredMetric
...
def do_train():
    ...
    if args.train_batch_size != 1:
        save_dir += '_bs' + str(args.train_batch_size)
    if args.dropout != 0.1:
    ...
    data_paths = {'train': os.path.join(args.data_dir, 'train.jsonl'),
    ...
    def get_data(paths, test=False, task3=False):
        pipe = EBDataPipe(tokenizer, args)
        _data_bundle = pipe.process_from_file(paths, test, task3)
        return _data_bundle

    if args.task3:
        if args.irgr:
            filter_path = {
                'dev': os.path.join(args.data_dir, 'filter/dev.jsonl'),
                'test': os.path.join(args.data_dir, 'filter/test.jsonl')
            }
            eval_data_bundle = get_data(paths=filter_path, _cache_fp=os.path.join(args.data_dir, args.test_data),
                                        _refresh=True, test=True, task3=True)
            logging.info("Using [IRGR] data to test.")
        else:
            filter_path = {
                'dev': os.path.join(args.data_dir, 'filter/dev.jsonl'),
                'test': os.path.join(args.data_dir, 'filter/test.jsonl')
            ...
            eval_data_bundle = get_data(paths=filter_path, _cache_fp=os.path.join(args.data_dir, args.test_data),
                                        _refresh=True, test=True, task3=True)
            logging.info("Using [Task3] data to test.")
    else:
        train_data_bundle = get_data(data_paths, test=False)
        train_data = train_data_bundle.get_dataset('train')
        train_dl = prepare_torch_dataloader(train_data, batch_size=args.train_batch_size,
                                            sampler=RandomSampler(train_data))

        eval_data_bundle = get_data(paths=data_paths, _cache_fp=os.path.join(args.data_dir, args.test_data), test=True)
        ...
        if args.filter:
            ...
            eval_data_bundle = get_data(paths=filter_path, _refresh=True, task3=args.task3,
                                        _cache_fp=os.path.join(args.data_dir, 'filter'), test=True)
            logging.info("Using [Eval-filter] data to test.")
    ...
    if args.do_train:
        ...
        metric = {"rlet": EBPredMetric(verbose=False)}
        ...
        @Trainer.on(Event.on_before_backward(every=500))
        ...
        trainer = Trainer(model=model,
                          ...
                          driver='torch',
                          device=device,
                          evaluate_dataloaders={"test": test_dl},
                          metrics=metric,
                          callbacks=callbacks,
                          output_mapping=None,
        ...
        if args.task3:
            pred_save_dir = os.path.join(pred_save_dir, 'result_t3')
            if not os.path.exists(pred_save_dir):
                os.makedirs(pred_save_dir, exist_ok=True)
            if args.irgr:
                pred_save_dir = os.path.join(pred_save_dir, para_str + 'pred_irgr.tsv')
            else:
                pred_save_dir = os.path.join(pred_save_dir, para_str + 'pred.tsv')
        else:
            if args.pred_para:
                pred_save_dir = os.path.join(pred_save_dir, para_str + 'para_pred.tsv')
            else:
                pred_save_dir = os.path.join(pred_save_dir, para_str + 'pred.tsv')
    ...
    if args.task3 or args.irgr:
        metrics = {"rlet": Task3EBPredMetric(test=True, out_path=pred_save_dir, has_end=True, verbose=True,
                                              pred_para=args.pred_para, inter_cache_path=args.inter_cache_path)}
    else:
        if args.pred_para:
            metrics = {"rlet": EBPredMetricWithPara(test=True, out_path=pred_save_dir, verbose=True,
                                                     inter_cache_path=args.inter_cache_path)}
        else:
            metrics = {"rlet": EBPredMetric(test=True, out_path=pred_save_dir, verbose=True)}
