...
class Policy(object):
    ...
    def update_state_get_new_nodes_offset(self, index, prob, offset, state_index, test=False, K_test=1):
        ...
        for i, idx in enumerate(index):
            ...
            # 要从上一轮保留哪些feature，删除已经使用过的句子，需要保留的feature按照new_index list进行存储
            new_index = []
            for j, cand_name in enumerate(prev_cand_name):
                if cand_name[0] in set(choice) or cand_name[1] in set(choice) or 'end' in set(cand_name):
                    continue
                else:
                    new_index.append(j)
            ...
            if self.inter_mode == 'model':
                ...
                if choice[0] == 'end':
                    new_inter_text = get_intermediate_sentence(sent2, sent1)
                else:
        ...
        return prev_state_indexes, prev_action_indexes, prev_feat_indexes, incremental_texts, is_end
    ...
    def get_reward(self, step_source, intermediate, args):
        ...
        for i, st in enumerate(self.state):
            choice = st['choice']
            if verbose:
                print("state:", st['action'])
                print("choice:", st['choice'])
                print("gold:", step_source)
            cur_reward = self.get_path_reward_align_accu_log(step_source, choice, i, args.discount)
            reward.append(cur_reward)
            if cur_reward > best_reward:
                best_path = st['choice']
        return reward, best_path
    ...
    def inference_get_reward(self, step_source, args):
        ...
        if not args.task3:
            ...
            return best_prob, best_path, best_all_sents, 0
